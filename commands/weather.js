...
module.exports = {
    data: new builders_1.SlashCommandBuilder()
        ...
        .setRequired(true)))
        .addSubcommand(subcommand => subcommand.setName("now")
        .setDescription("한국 날씨만 검색 가능합니다.")
        .addStringOption(option => option
        .setName("location")
        .setDescription("한국 지역만 검색 가능합니다. (도 시 구 동 면 읍 등등)")
        ...
        .setDescription("한국 지역만 검색 가능합니다. (도 시 구 동 면 읍 등등)")
        .setRequired(true))),
    run: ({ _client, interaction }) => __awaiter(void 0, void 0, void 0, function* () {
        const location = interaction.options.getString(`location`);
        const geo = yield get_city_geo_info(location);
        if (geo === undefined || geo === null)
            return interaction.editReply(`도시를 찾지 못했습니다!\n` +
                `검색어: ${location}\n` +
                `한 단어씩 검색해 보십시오.`);
        const local_loc_name = geo["local_names"]["ko"];
        const lat = geo["lat"];
        const lon = geo["lon"];
        const weather_info = yield get_weather_info(lat, lon);
        ...
        if (interaction.options.getSubcommand() === 'forecast') {
            ...
            embed
                .setTitle("제로투 봇 기상정보")
                .setDescription(`**${local_loc_name}의 기상예보는 다음과 같습니다.**\n` +
            ...
            embed
                .setTitle("제로투 봇 기상정보")
                .setDescription(`**${local_loc_name}의 현재 날씨정보는 다음과 같습니다.**\n` +
                `${time} 기준\n` +
                `날씨: **${weather_desc}**\n` +
                `기온: \`${temp}C\`\n` +
                `체감온도: \`${feels_like}C\`\n` +
            ...
            const air_pollution = yield get_air_pollution(lat, lon);
...
function get_city_geo_info(city_name) {
...
function get_air_pollution(lat, lon) {
    return __awaiter(this, void 0, void 0, function* () {
...
var Pollutant;
(function (Pollutant) {
    Pollutant[Pollutant["NO2"] = 0] = "NO2";
    Pollutant[Pollutant["PM10"] = 1] = "PM10";
    Pollutant[Pollutant["O3"] = 2] = "O3";
    Pollutant[Pollutant["PM2_5"] = 3] = "PM2_5";
})(Pollutant || (Pollutant = {}));
var Standards;
(function (Standards) {
    Standards["\uC88B\uC74C"] = "\uC88B\uC74C";
...
function get_air_pollution_standards(pollutant, quality) {
    switch (pollutant) {
        case Pollutant.NO2:
            if (quality <= 50)
                return Standards.좋음;
                ...
                return Standards.그저그럼;
            else if (quality <= 400)
                return Standards.나쁨;
            else if (quality > 400)
                return Standards.매우나쁨;
            break;
        case Pollutant.PM10:
            if (quality <= 25)
                return Standards.좋음;
